...
int
vm2k(pagetable_t pagetable,pagetable_t kernel_pagetable,uint64 oldsz,uint64 newsz)
{
  //printf("bef vm2k");
  pte_t *from,*to;
  ...
  for(uint64 va=PGROUNDUP(oldsz);va<newsz;va+=PGSIZE){
    if((from=walk(pagetable,va,0))==0)
      panic("pte not exist");
    if((to=walk(kernel_pagetable,va,1))==0)
      panic("walk failed");
    *to=(*from)&(~PTE_U);
  }
  return 0;
...
void
r_print(pagetable_t pagetable,int depth)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      ...
      printf("%d: pte %p pa %p\n",i,pte,child);
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0)
        r_print((pagetable_t)child,depth+1);
    }
  }
}
